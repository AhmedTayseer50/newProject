...
const { getAdmin } = require('./_lib/firebaseAdmin') as {
  getAdmin: () => any;
};
...
async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    console.log('[player-session] init firebase...');
    const admin = getAdmin();

    console.log('[player-session] verifying token...');
    const decoded = await admin.auth().verifyIdToken(idToken);
    const uid = decoded.uid;

    const enrollPath = `enrollments/${uid}/${courseId}`;
    console.log('[player-session] enrollPath:', enrollPath);

    const enrollmentSnap = await admin.database().ref(enrollPath).get();
    const isEnrolled = enrollmentSnap.exists();

    console.log('[player-session] isEnrolled:', isEnrolled);
    ...
    console.error('[player-session] ERROR:', err?.message);
    console.error(err);
    res.status(500).send(err?.message || 'FUNCTION_INVOCATION_FAILED');
  }
}

// ✅ CommonJS export عشان Vercel يشغّله كـ CJS بدون import/esm مشاكل
